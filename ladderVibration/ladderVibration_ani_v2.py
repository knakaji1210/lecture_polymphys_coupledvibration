...
fig = plt.figure()
ax = fig.add_subplot(111, xlim=(0, L), ylim=(-L/5, 0.6*L))
ax.grid()
...
if ini == "s": 
    savefile = './gif/ladderdVibration_single_(ini{0},show{1},k={2:.1f},m={3:.1f}).gif'.format(mode,mode2show,k,m)
    ani.save(savefile, writer='pillow', fps=fps)
elif ini == "m":
    savefile = './gif/ladderdVibration_multiple_(show{0},k={1:.1f},m={2:.1f}).gif'.format(mode2show,k,m)
    ani.save(savefile, writer='pillow', fps=fps)
elif ini == "d":
    savefile = './gif/ladderdVibration_delta_(show{0},k={1:.1f},m={2:.1f}).gif'.format(mode2show,k,m)
    ani.save(savefile, writer='pillow', fps=fps)
else:
    pass
