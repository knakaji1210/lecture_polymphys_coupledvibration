...
peri1_template = r'$\tau$ = {0:.2f} s, $\omega_r$ = {1:.2f} s$^{{-1}}$,'.format(tau,afreq)
...
def init():                 # FuncAnimationでinit_funcで呼び出す
    time_text.set_text('')
    return input, rod, triangle, mass, time_text, peri2_text, amp_o_text, pha_o_text
...
savefile = './gif/forced_harmonicOsci_freqSweep_(k={0:.1f},m={1:.1f},c={2:.1f}).gif'.format(k,m,c)
